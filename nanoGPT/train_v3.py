...
def get_batch(split):
    ...
    if split == 'train':
        # get the CIF start indices
        if underrep_p is not None and np.random.random() < underrep_p:
            ix = cif_start_indices_underrep[torch.randperm(len(cif_start_indices_underrep))[:batch_size]]
        else:
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if (validate and losses['val'] < best_val_loss) or always_save_checkpoint:
            best_val_loss = losses['val'] if validate else 0.
            if iter_num > 0:
                checkpoint = {
