...
class GPTRegressor(nn.Module):
    ...
    def get_num_params(self, non_embedding=True):
        ...
        The token embeddings do too, since there is no parameter sharing in this model.
        """
    ...
    def forward(self, idx, targets=None):
        ...
        # use the output of the last token only
        logits = logits[:, -1]  # shape (b, 1)

        # if targets are provided, calculate the loss
        if targets is not None:
            loss_fn = nn.L1Loss()
            targets = targets.to(device)  # move targets to the same device as logits
            loss = loss_fn(logits.view(-1), targets.view(-1))  # flatten logits and targets before calculating loss
            return logits, loss
    ...
    def init_from_pretrained(self, pretrained_model):
        # copy the weights from the pretrained model's transformer layers to the new model
        self.transformer.load_state_dict(pretrained_model.transformer.state_dict())

    def freeze_transformer_weights(self):
        """
        The frozen parameters won't be updated during training, and only the
        self.lm_head parameters will be trained.
        """
