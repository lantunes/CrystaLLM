...
class MCTSDefaults:
    ...
    scorer: str = "zmq"  # supported values: 'zmq', 'random', "PytorchChgnet"
...
if __name__ == "__main__":
    ...
    device_type = "cuda:0" if "cuda" in C.device else "cpu"  # for later use in torch.autocast
    ...
    checkpoint = torch.load(ckpt_path, map_location=device_type) #C.device
    ...
    elif C.scorer == "PytorchChgnet":
        scorer_device = "cuda:1" if "cuda" in C.scorer_device else "cpu"
        cif_scorer = PytorchChgnetScorer(host_device=device_type, scorer_device=scorer_device, model_name=C.model_name)
